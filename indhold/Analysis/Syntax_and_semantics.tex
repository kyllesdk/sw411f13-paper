...
SQRT, also known as square-root($\sqrt{}$) is a simple rule to allow any positive arithmetic expression, $v_{1}$ to be squared, where $v$ obviously is $\sqrt{v_{1}}$. Like on the transition rule for division is an extra side-note, which indicates that the number that is to be square-rooted is either a zero or a positive number, as the source language does not going to allow imaginary numbers. \\
