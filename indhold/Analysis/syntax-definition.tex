...
\begin{itemize}
	...
	\item If-statement - A statement can consist of an if-statement which is a condition. If the boolean expression evaluates to true another statement is called or declare a variable etc. If the condition evaluates to false then the program checks if the if-statement has an ``elseif'' or an ``else'' statement and then evaluates that.
